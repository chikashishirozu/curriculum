...
class MemoryGame:
    def __init__(self, root):
        ...
        self.root.geometry("1800x1350")
        self.root.minsize(1800, 1350)
        
        font_path = "/home/hiroppy123/.local/share/fonts/NotoSansJP-Regular.ttf"
        self.pil_font = ImageFont.truetype(font_path, size=32)
    ...
    def load_assets(self):
        ...
        for i in range(1, 10):  # 仮に9種類のカードがあるとする
            try:
                image = Image.open(f"cards/card_{i}.png").resize((226, 332))  # 適切なサイズに調整
                # 画像ファイルを読み込み
        ...
        # カードの裏画像を読み込む
        try:     
            back_image = Image.open("cards/card_back.png").resize((226, 336))  # 適切なサイズに調整
    ...
    def create_card_image(self, color):
        ...
        try:
            font = ImageFont.truetype("./NotoSansJP-Regular.ttf", 40)  # 24ptに変更
        except:
    ...
    def show_menu(self):
        ...
        if self.custom_font.cget("family") == "liberation sans":
            self.custom_font = tkfont.Font(family="liberation serif", size=28)
        ...
        # PIL で画像に文字を描画
        if self.pil_font:
            img = Image.new("RGB", (300, 90), color="silver")
            draw = ImageDraw.Draw(img)
            draw.text((20, 10), "神経衰弱GAME", font=self.pil_font, fill="navy")
        ...
        try:        
            ...
            bg_image = Image.open(bg_image_path).resize((1800, 1350))  # 適切なサイズに調整
        ...
        # フォント設定
        font_settings = ("liberation sans", 24)

        tk.Label(settings_frame, text="プレイヤー人数:", 
                font=("liberation sans", 24), width=17, height=2).grid(row=0, column=0, sticky="e")
        self.players_var = tk.StringVar(value=str(self.default_settings['players']))
        tk.Entry(settings_frame, textvariable=self.players_var, width=7, font=font_settings).grid(row=0, column=1)
       
        tk.Label(settings_frame, text="行数:", 
                font=("liberation sans", 24), width=17, height=2).grid(row=1, column=0, sticky="e")
        self.rows_var = tk.StringVar(value=str(self.default_settings['rows']))
        tk.Entry(settings_frame, textvariable=self.rows_var, width=7, font=font_settings).grid(row=1, column=1)
        
        tk.Label(settings_frame, text="列数:", 
                font=("liberation sans", 24), width=17, height=2).grid(row=2, column=0, sticky="e")
        self.cols_var = tk.StringVar(value=str(self.default_settings['cols']))
        tk.Entry(settings_frame, textvariable=self.cols_var, width=7, font=font_settings).grid(row=2, column=1)
        
        # スタートボタン
        tk.Button(self.menu_frame, text="ゲーム開始", 
                 command=self.start_game, bg="blue", fg="white", font=("liberation sans", 30), width=14, height=2).pack(pady=5, anchor="center")                
