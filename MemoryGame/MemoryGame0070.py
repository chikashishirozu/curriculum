...
class MemoryGame:
    ...
    def load_assets(self):
        ...
        for i in range(1, 10):  # 仮に9種類のカードがあるとする
            try:
                ...
                # 画像ファイルを読み込み
                image = round_corners(image, radius=18)
        ...
        try:     
            ...
            back_image = round_corners(back_image, radius=18)
    ...
    def show_menu(self):
        ...
        if self.custom_font.cget("family") == "liberation sans":
            self.custom_font = tkfont.Font(family="sazanami gothic", size=20)
        ...
        # PIL で画像に文字を描画
        if self.pil_font:
            img = Image.new("RGB", (440, 115), color="#00CED1")
            draw = ImageDraw.Draw(img)
            draw.text((18, 10), "神経衰弱GAME", font=self.pil_font, fill="#191970")
    ...
    def show_game(self):
        ...
        # メニューボタン
        tk.Button(self.game_frame, text="メニューに戻る", bg="#40E0D0", font=self.custom_font, command=self.show_menu).pack(padx=10, pady=(0, 40))
